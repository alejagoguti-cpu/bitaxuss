...
function ReferenceFrame({ eyebrow, title, children, conclusion, variant = "dark" }: { eyebrow: string; title: string; children: ReactNode; conclusion?: string; variant?: "light" | "dark" }) {
  return <section className={`reader-reference-frame reader-reference-${variant}`}><p className="reader-reference-eyebrow">{eyebrow}</p><h3>{title}</h3>{children}{conclusion && <p className="reader-reference-conclusion">{conclusion}</p>}</section>;
}
